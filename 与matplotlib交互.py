...
class QmyMainWindow(QMainWindow):
    ...
    def __drawFigure(self):  ##绘图
        ...
        ax1 = self.__fig.add_subplot(1, 2, 1)  # matplotlib.axes.Axes类
        ax1.plot(t, y1, 'r-o', label="sin", linewidth=1, markersize=5)
        ax1.plot(t, y2, 'b:', label="cos", linewidth=2)
        ax1.set_xlabel('X轴')  # x轴标题
        ax1.set_ylabel('Y轴', fontsize=14)  # y轴标题
        ax1.set_xlim([0, 10])
        ax1.set_ylim([-1.5, 1.5])
        ax1.set_title("曲线")  # 子图标题
        ax1.legend()  # 自动创建图例

        ax2 = self.__fig.add_subplot(1, 2, 2)  # matplotlib.axes.Axes类
